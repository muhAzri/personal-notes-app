...
function Card({ className, ...props }: React.ComponentProps<"div">): JSX.Element {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 flex flex-col gap-6 rounded-xl border border-gray-200 dark:border-slate-700 py-6 shadow-sm transition-colors",
        className
      )}
      {...props}
    />
